...
public class DeleteCupomUseCaseImpl extends DeleteCupomUseCase {

    private final CupomDomainGateway gateway;

    public DeleteCupomUseCaseImpl(CupomDomainGateway gateway) {

        this.gateway = gateway;
    }

    @Override
    public Either<Notification, DeleteCupomOutput> execute(DeleteCupomCommand aCupomCommand) {
        ...
        return delete(aCupomDB.get());
    }

    @Transactional
    private Either<Notification, DeleteCupomOutput> delete(Cupom aCupom) {

        return Try(() -> {
            gateway.delete(aCupom);
            return DeleteCupomOutput.from(aCupom);
